...
func (header *Header) ContainsUTF8Key() { header.utf8Key = true }

func (header *Header) IsContainsUTF8Key() bool { return header.utf8Key }

func (header *Header) keyToLower(k string) string {
	header.buf.Reset()
	if header.utf8Key {
		for _, r := range k {
			header.buf.WriteRune(unicode.ToLower(r))
		}
	} else {
